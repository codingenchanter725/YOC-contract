...
describe("Yocswap Test", function () {
  let token0;
  let token1;
  let WETH;
  let factory;
  ...
  beforeEach(async () => {
    ...
    router = await YocswapRouter.deploy(factory.address, WETH.address);
    ...
    WETHPair = await ethers.getContractAt("IYocswapPair", WETHPairAddress);
  });
